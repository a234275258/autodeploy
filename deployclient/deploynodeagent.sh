...
progpath=/opt/deployclient-prod  # 程序目录
progfile=deployclient.py  # 程序文件
confile=deployclient.conf # 程序配置文件
state=0 # 状态值，用于判断svn是否符合要求
cd ${progpath}
...
start(){
  ...
  if ps -ef | grep ${progfile}|grep -v grep &>/dev/null; then
    action  "Start autodeploy nodeagent..."  /bin/true
  else
    ...
    if ps -ef | grep ${progfile}|grep -v grep &>/dev/null; then
     ...
     action "Start autodeploy nodeagent..."  /bin/false
    fi
  fi
}
...
stop(){
  if ps -ef | grep ${progfile} |grep -v grep &>/dev/null; then
    ...
    if [ $? -eq 0 ];then
      action "Stop autodeploy nodeagent..."  /bin/true
    else
      action "Stop autodeploy nodeagent..."  /bin/false
    fi
  else
    action "Stop autodeploy nodeagent..."  /bin/false
  fi
...
status(){
  if ps -ef | grep ${progfile} &>/dev/null; then
      echo  "autodeploy nodeagent is running.." 
    else
      echo  "autodeploy nodeagent is not running.." 
    fi
}
